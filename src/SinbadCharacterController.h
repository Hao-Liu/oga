...
#define JUMP_ACCEL 80.0f       // character jump acceleration in upward units per squared second
...
private:
	...
	void updateBody(Real deltaTime)
	{
		...
		{
			RaySceneQuery* rayQuery = mCamera->getSceneManager()->createRayQuery(Ray(mBodyNode->getPosition(), Vector3::NEGATIVE_UNIT_Y));
			rayQuery->setWorldFragmentType(SceneQuery::WFT_PLANE_BOUNDED_REGION);
			rayQuery->setSortByDistance(true, 10);
			RaySceneQueryResult& queryResult = rayQuery->execute();
			RaySceneQueryResult::iterator i = queryResult.begin();

			int n=0;
			while(i!=queryResult.end()) 
			{
				if (i->movable)
				{
					std::cout << i->movable->getName();
				}
				else
				{
					std::cout << "w:";
				}
				std::cout	<< std::setw(10) << i->distance << " ";
			 	i++;n++;
			}
			std::cout << n << "\n";
			i = queryResult.begin();
			while(i!=queryResult.end() && i->distance == 0 ) 
			{
				i++;
			}
			if (i->movable)
				if (i->movable->getParentSceneNode() == mBodyNode)
					i++;
			...
			if(i != queryResult.end())
			{
				if (i->distance >= 5.0)
				{
					...
					mBodyNode->setPosition(mBodyNode->getPosition() + Vector3(0,mVerticalVelocity * deltaTime,0));
					if (i->distance > 6 && (mTopAnimID == ANIM_IDLE_TOP || mTopAnimID == ANIM_RUN_TOP))
					{
				...
				{
					mVerticalVelocity = 0;
					mBodyNode->setPosition(mBodyNode->getPosition() - Vector3(0,i->distance - 5.0,0));
		...
		{
			RaySceneQuery* rayQuery = mCamera->getSceneManager()->createRayQuery(Ray(mBodyNode->getPosition(), Vector3::NEGATIVE_UNIT_Z));
			rayQuery->setWorldFragmentType(SceneQuery::WFT_PLANE_BOUNDED_REGION);
			rayQuery->setSortByDistance(true, 10);
			RaySceneQueryResult& queryResult = rayQuery->execute();
			RaySceneQueryResult::iterator i = queryResult.begin();

			//while(i!=queryResult.end() && i->distance == 0) i++;
			while(i!=queryResult.end() && i->movable) i++;

			if(i != queryResult.end())
			{
				if (i->distance < 5.0)
				{
					mBodyNode->setPosition(mBodyNode->getPosition() - Vector3(0,0,i->distance - 5.0));
				}
			}
		}

		{
			RaySceneQuery* rayQuery = mCamera->getSceneManager()->createRayQuery(Ray(mBodyNode->getPosition(), Vector3::UNIT_Z));
			rayQuery->setWorldFragmentType(SceneQuery::WFT_PLANE_BOUNDED_REGION);
			RaySceneQueryResult& queryResult = rayQuery->execute();
			RaySceneQueryResult::iterator i = queryResult.begin();
		...
		{
			RaySceneQuery* rayQuery = mCamera->getSceneManager()->createRayQuery(Ray(mBodyNode->getPosition(), Vector3::NEGATIVE_UNIT_X));
			rayQuery->setWorldFragmentType(SceneQuery::WFT_PLANE_BOUNDED_REGION);
			rayQuery->setSortByDistance(true, 10);
			RaySceneQueryResult& queryResult = rayQuery->execute();
			RaySceneQueryResult::iterator i = queryResult.begin();
			...
			if(i != queryResult.end())
			{
				if (i->distance < 5.0)
				{
					mBodyNode->setPosition(mBodyNode->getPosition() - Vector3(i->distance - 5.0,0,0));
				}
			}
		}

		{
			RaySceneQuery* rayQuery = mCamera->getSceneManager()->createRayQuery(Ray(mBodyNode->getPosition(), Vector3::UNIT_X));
			rayQuery->setWorldFragmentType(SceneQuery::WFT_PLANE_BOUNDED_REGION);
			rayQuery->setSortByDistance(true, 10);
			RaySceneQueryResult& queryResult = rayQuery->execute();
			RaySceneQueryResult::iterator i = queryResult.begin();
			...
			if(i != queryResult.end())
			{
				...
				//std::cout << StringConverter::toString(i->distance) << "+X \n";
			}
		}
	...
	void updateAnimations(Real deltaTime)
	{
		...
		else if (mBaseAnimID == ANIM_JUMP_START)
		{
			if (mTimer >= mAnims[mBaseAnimID]->getLength())
			{
				...
				mBodyNode->setPosition(mBodyNode->getPosition() + Vector3(0, 0.5 ,0));
			}
		}
	...
	void updateCameraGoal(Real deltaYaw, Real deltaPitch, Real deltaZoom)
	{
		...
		// bound the pitch
		if (!(mPivotPitch + deltaPitch > 89 && deltaPitch > 0) &&
			!(mPivotPitch + deltaPitch < -89 && deltaPitch < 0))
		{
